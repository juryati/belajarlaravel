...
class SiswaController extends Controller
{
    ...
    public function store(Request $request)
    {
        $rules = [
            ...
            'alamat' => 'required',
            'no_telp' => 'required',
            'id_kelas' => 'required|exists:tabel_kelas',
			'foto' => 'required|mimes:png|max:512'
        ];
        $this->validate($request, $rules);
    	$input = $request->all();
		if ($request->hasFile('foto') && $request->file('foto')->isValid){
			filename = $input['nis'] . "." . $request->file('foto')->getClinetOriginalExtensi();
    ...
    {
        $data['result'] = \App\Siswa::where('id_kelas', $id)->first();
